...
export default function BuyerLibraryPage() {
  ...
  const handleDownload = async (purchaseId: string, assetId: string, fileName: string) => {
    try {
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-6">
            {purchases.map((purchase) => (
              <Card key={purchase.id} className="border-border/50 shadow-sm hover:shadow-md transition-all group overflow-hidden">
                <CardContent className="p-0">
                  <div className="flex flex-col lg:flex-row lg:items-center">
                    ...
                    <div className="flex-1 p-6 space-y-4">
                      <div className="flex items-start justify-between gap-4">
                        <div className="space-y-1">
                          ...
                          <h3 className="text-xl font-black group-hover:text-primary transition-colors">{purchase.dataset_title}</h3>
                        </div>
                      </div>
                    ...
                    <div className="lg:w-[450px] bg-muted/30 p-6 border-t lg:border-t-0 lg:border-l space-y-4">
                      <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Download Assets</h4>
                      <div className="grid grid-cols-3 gap-3">
                        {purchase.assets.map((asset) => (
                          <button
                            key={asset.id}
                            onClick={() => handleDownload(purchase.id, asset.id, `${purchase.dataset_title}.${asset.file_format}`)}
                          ...
                          >
                            <Download className={`w-4 h-4 text-primary transition-transform ${downloadingAssetId === asset.id ? 'animate-bounce' : 'group-hover/btn:translate-y-0.5'}`} />
                            <span className="text-xs font-black">{asset.file_format.toUpperCase()}</span>
                            <span className="text-[10px] text-muted-foreground">{formatFileSize(asset.file_size_bytes)}</span>
                          </button>
                        ))}
                      </div>
                    </div>
