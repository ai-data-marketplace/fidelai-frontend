...
export function DatasetDetails({ dataset }: DatasetDetailsProps) {
  ...
  const purchaseStatus = String(dataset.purchase_status || "")
    .toLowerCase()
    .trim();
  const purchaseButtonState = (() => {
    if (purchaseStatus === "pending")
      return { label: "Pending Payment", disabled: true };
    if (purchaseStatus === "active")
      return { label: "In Library", disabled: false };
    if (
      ...
      <div className="flex flex-col lg:flex-row justify-between gap-8 bg-card border rounded-3xl p-8 shadow-xl shadow-primary/5">
        <div className="space-y-4 max-w-2xl">
          <div className="flex items-center gap-3">
            <Badge
              variant="outline"
              className="px-3 py-1 font-black text-primary border-primary/20 bg-primary/5 uppercase tracking-widest text-[10px]"
            >
              {dataset.domain}
            </Badge>
              ...
              <div className="relative flex justify-between gap-2 max-w-4xl mx-auto">
                {pipelineSteps.map((step, idx) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-500 scale-90 ${
                          isCompleted
                      ...
                      <span
                        className={`text-[10px] font-bold uppercase tracking-wider text-center max-w-[80px] ${
                          isCompleted
                            ? "text-primary"
                ...
                <div
                  ...
                  style={{
                    width: `${(currentStepIndex / (pipelineSteps.length - 1)) * 100}%`,
                  }}
                />
              </div>
            </CardContent>
                ...
                {[
                  {
                    ...
                    value: dataset.metrics.total_documents.toLocaleString(),
                  },
                  {
                    label: "Chunk Count",
                    value: dataset.metrics.chunk_count.toLocaleString(),
                  },
                  {
                    label: "Token Count",
                    value: formatTokenCount(dataset.metrics.token_count),
                  },
                  {
                    label: "Avg QC Score",
                    value: `${Math.round(dataset.metrics.avg_qc_score * 100)}%`,
                  },
                  {
                    label: "Annotation Coverage",
                    value: `${Math.round(dataset.metrics.annotation_coverage * 100)}%`,
                  },
                  {
              ...
              <div className="grid grid-cols-2 gap-2">
                {dataset.assets?.length ? (
                  dataset.assets.map((asset) => {
                    const AssetIcon =
                      asset.file_format === "csv" || asset.file_format === "tsv"
                        ? Table
