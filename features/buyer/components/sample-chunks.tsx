        ...
        {samples.length === 0 ? (
          <Card className="border-dashed bg-muted/20">
            <CardContent className="p-12 text-center text-muted-foreground flex flex-col items-center gap-3">
              ...
              <p className="font-medium">Sample data is unavailable for this dataset.</p>
              <p className="text-xs">Purchase required for the full sample set and assets.</p>
            </CardContent>
          </Card>
        ) : (
                  ...
                  <div className="flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x">
                    {/* Metadata Sidebar */}
                      ...
                      <div className="space-y-1">
                        <span className="text-[10px] font-bold text-muted-foreground uppercase opacity-60">Label</span>
                      ...
                      <div className="space-y-1">
                        ...
                        <Badge className={`text-[10px] px-2 py-0 h-5 font-black ${
                          sample.quality_score >= 0.95 ? 'bg-emerald-500/20 text-emerald-700' : 'bg-primary/20 text-primary'
...
{JSON.stringify({ text: sample.text, label: sample.label }, null, 2)}
                        </pre>
                      </div>
                    </div>
                  </div>
        ...
        {/* After the last sample show a single locked/purchase card */}
        {samples.length > 0 && (
          <motion.div
              ...
              <CardContent className="p-6 text-center">
                <div className="flex flex-col items-center gap-3">
                  <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white border shadow-sm">
                    ...
                    <span className="text-sm font-bold uppercase tracking-widest text-primary">Content Locked</span>
                  </div>
                  <h4 className="font-semibold">Want to see more?</h4>
                  <p className="text-sm text-muted-foreground">Purchase the dataset to unlock the full sample set and downloadable assets.</p>
