...
import { Database, Users, Wallet, ArrowRight } from "lucide-react";
          ...
          <Card className="relative h-full min-h-[112px] overflow-hidden border-border/40 bg-card shadow-sm transition-all group hover:border-primary/50">
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                <div
                  className={`${action.color} text-white p-3 rounded-2xl shadow-lg shadow-black/5`}
                >
                  {action.icon}
                </div>
            ...
            <div
              className={`absolute top-0 right-0 w-24 h-24 ${action.color} opacity-[0.03] -mr-8 -mt-8 rounded-full`}
            />
          </Card>
        </Link>
      ))}
