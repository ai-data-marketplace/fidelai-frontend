...
export function NlpWorkspace({ taskId }: NlpWorkspaceProps) {
  ...
  const {
    ...
    isError: taskError,
  } = useNlpTaskDetail(taskId);
  const { data: progress, isLoading: progressLoading } =
    useNlpTaskProgress(taskId);
  ...
  useEffect(() => {
    if (task?.chunks && task.chunks.length > 0 && !initializedRef.current) {
      const firstUnannotatedIndex = task.chunks.findIndex(
        (chunk) => chunk.previous_annotation === null,
      );
  ...
  useEffect(() => {
    ...
    if (currentChunk.previous_annotation) {
      ...
      const previousLabel =
        labels[taskType] ?? Object.values(labels)[0] ?? null;
      const parsedConfidence = Number.parseFloat(
        String(currentChunk.previous_annotation.confidence_score),
      );
  ...
  if (
    (taskError || !task || !task.chunks || task.chunks.length === 0) &&
    !isComplete &&
    !progressLoading
  ) {
    return (
  ...
  const progressPercent =
    ...
    (task?.chunks?.length
      ? ((currentIndex + 1) / task.chunks.length) * 100
      : 0);
          ...
          <div className="space-y-7 flex-1 pb-6">
            <div className="space-y-3">
              <label className="text-sm font-bold text-foreground">
                Select the appropriate label for this text:
              </label>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          ...
          <div className="p-4 rounded-xl border bg-emerald-500/5 border-emerald-500/20 text-center col-span-2">
            ...
            <p className="text-xl font-bold text-emerald-600">
              {progress?.total_chunks ?? task?.chunks?.length ?? 0}
            </p>
          </div>
        </div>
